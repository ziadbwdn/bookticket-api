...
func NewTicketService(db *gorm.DB, ticketRepo contract.TicketRepository, eventRepo contract.EventRepository, activityService contract.UserActivityService, logger logger.Logger) contract.TicketService {
	// panic checking
	if db == nil {
		panic("database connection must not be nil for TicketService")
	}
	if ticketRepo == nil {
...
func (s *TicketServiceImpl) PurchaseTicket(ctx context.Context, ticket *entities.Ticket, logCtx entities.ActivityLogContext) (*entities.Ticket, *exception.AppError) {
	tx := s.db.Begin()
	if tx.Error != nil {
		s.logger.Error(ctx, "Failed to begin transaction for PurchaseTicket", tx.Error)
	...
	if err := tx.Commit().Error; err != nil {
		...
		s.logger.Error(ctx, "Failed to commit transaction for PurchaseTicket", err)
...
func (s *TicketServiceImpl) UpdateTicketStatus(ctx context.Context, id utils.BinaryUUID, req *dto.UpdateTicketStatusRequest, reason string, logCtx entities.ActivityLogContext) (*entities.Ticket, *exception.AppError) {
	tx := s.db.Begin()
	if tx.Error != nil {
		s.logger.Error(ctx, "Failed to begin transaction for UpdateTicketStatus", tx.Error)
		return nil, exception.NewInternalError("failed to begin transaction", tx.Error)
	}
	...
	if err := tx.Commit().Error; err != nil {
		tx.Rollback() // Ensure rollback if commit fails
		s.logger.Error(ctx, "Failed to commit transaction for UpdateTicketStatus", err)
		return nil, exception.NewInternalError("failed to commit transaction", err)
	}
...
func (s *TicketServiceImpl) DeleteTicket(ctx context.Context, id utils.BinaryUUID, logCtx entities.ActivityLogContext) *exception.AppError {
	tx := s.db.Begin()
	if tx.Error != nil {
		s.logger.Error(ctx, "Failed to begin transaction for DeleteTicket", tx.Error)
	...
	if err := tx.Commit().Error; err != nil {
		tx.Rollback() // Ensure rollback if commit fails
		s.logger.Error(ctx, "Failed to commit transaction for DeleteTicket", err)
