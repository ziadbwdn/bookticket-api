...
import (
	...
	"root-app/internal/database/seeder"
...
func main() {
	...
	if err := seeder.RunSeeder(db); err != nil {
		appLogger.Error(context.Background(), "Failed to run database seeder", err)
		log.Fatalf("Failed to run database seeder: %v", err)
	}
	appLogger.Info(context.Background(), "Database seeder completed successfully.")
